...
fun preprocess_prog (e) =
    case e of
    LineSegment (x1,y1,x2,y2) =>    let
                                    fun make_point (x1,y1,x2,y2) =
                                        if real_close(x1,x2) andalso real_close(y1,y2)
                                        then Point (x1,y1)
                                        else LineSegment(x1,y1,x2,y2)
                                    fun swap_point (x1,y1,x2,y2) = 
                                        if real_close(x1,x2) andalso y2 < y1 orelse x2 < x1
                                        then (x2,y2,x1,y1)
                                        else (x1,y1,x2,y2)
                                    in
                                        make_point(swap_point(x1,y1,x2,y2))
                                    end
